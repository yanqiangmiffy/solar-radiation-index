...
def creat_fea(data_df):
    ...
    data_df = add_poly_features(data_df, co)
    for i in range(0, len(data_df)):
        if i < 1:
            data_df.loc[i, 'old_辐照度_mean'] = data_df.loc[i, '辐照度_mean']
        elif i < 2:
            data_df.loc[i, 'old_辐照度_mean'] = (data_df.loc[i, '辐照度_mean'] + data_df.loc[i - 1, '辐照度_mean']) / 2
        else:
            data_df.loc[i, 'old_辐照度_mean'] = (data_df.loc[i, '辐照度_mean'] + data_df.loc[i - 1, '辐照度_mean'] + data_df.loc[
                i - 2, '辐照度_mean']) / 3
    return data_df
